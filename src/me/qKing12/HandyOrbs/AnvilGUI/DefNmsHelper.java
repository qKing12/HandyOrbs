...
public class DefNmsHelper implements NmsHelper {
    ...
    static {
        ...
        try {
            ...
            if(version.contains("1_15") || version.contains("1_14")){
                packetPlayOutOpenWindowConstructor = packetPlayOutOpenWindowClass.getConstructor(Integer.TYPE, getNMSClass("Containers"), getNMSClass("IChatBaseComponent"));
            }
            else{
            ...
            try {
                ...
                if (version.contains("1_13") || version.contains("1_14"))
                    blockA = blockClass.getMethod("m");
                else
                    blockA = blockClass.getMethod("a");
     ...
     */
    @Override
    public void sendPacketOpenWindow(Player player, int containerId, Object container) {
            ...
            try {
                playerConnectionSendPacket.invoke(//.sendPacket(
                        ...
                        packetPlayOutOpenWindowConstructor.newInstance(//new PlayOutOpenWindow(
                                containerId,
                                getNMSClass("Containers").getField("ANVIL").get(null),
                                chatMessageConstructor.newInstance("", new Object[]{})
                        )//)
                );//);
            } catch (Exception e) {
                handleException(e);
     ...
     */
    @Override
    public Object newContainerAnvil(Player player, int containerId) {
        /*
        ...
        try {
            ...
            if(version.contains("1_15") || version.contains("1_14")){
                Class containerAccess = getNMSClass("ContainerAccess");
                Method at = containerAccess.getMethod("at", getNMSClass("World"), getNMSClass("BlockPosition"));
                obj = containerAnvilConstructor.newInstance(
                        containerId,
                        playerInventory.get(nms),
                        at.invoke(null, playerWorld.get(nms), blockPositionZero)
                );
            }
            else {
                obj = containerAnvilConstructor.newInstance(
