...
public class Database {
    ...
    public static void saveToFile(Main plugin) throws IOException {
        ...
        for(ArrayList<Orb> orbs : ConfigLoad.orbsManager.values())
            for(Orb orb : orbs)
    ...
    public static void loadFromFile(Main plugin) throws IOException {
        ...
        if(!database.exists()) {
            ...
            for(String loc : transitCrystal) {
                ...
                Orb newOrb = new Orb(loc2, null);

                if(loc2.getChunk().isLoaded())
                    ConfigLoad.orbs.add(newOrb);

                String toCheck = loc2.getChunk().toString();

                if(!ConfigLoad.orbsManager.containsKey(toCheck)){
                    ArrayList<Orb> newOrbs = new ArrayList<>();
                    newOrbs.add(newOrb);
